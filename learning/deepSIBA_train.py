    ...
    for i in outer_loop:
        ...
        while n < train_params["N_ensemble"]:
            ...
            history = deepsiba.fit_generator(trainGen,
                        ...
                        shuffle = True,
                        callbacks= [term, rlr])
            if history.history["r_square"][NUM_EPOCHS-1] < 0.7:
                history = deepsiba.fit_generator(trainGen,
                            steps_per_epoch= ceil(NUM_TRAIN/bs),
                            epochs = 10,
                            verbose = 1,
                            ...
                            callbacks= [term, rlr])
                check = 9
            if history.history["r_square"][check] >= 0.7:
                ...
                if (len(y_pred[np.where(y_pred <= train_params["prec_threshold"])])>0):
                    get = model_evaluate(y_pred,Y_cold,train_params["prec_threshold"],df_cold)
                ...
                np.save(train_params["output_dir"] + "/" + "fold_%s/cold/mu/"%i + "cold_mu_%s.npy"%n, cold_pred[0])
                cold_preds_sigmas.append(cold_pred[1])
                np.save(train_params["output_dir"] + "/" + "fold_%s/cold/sigma/"%i + "cold_sigma_%s.npy"%n, cold_pred[1])
                n = n + 1
